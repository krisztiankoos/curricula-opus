---
module: a2-14
title: "Aspect Pairs Deep Dive"
subtitle: "Prefixation Patterns and Common Pairs"
version: "1.0"
phase: "A2.2"
pedagogy: "PPP"
duration: "60 min"
transliteration: none
tags:
  - grammar
  - aspect
  - word-formation
  - prefixes
grammar:
  - prefixation patterns
  - suffixation patterns
  - suppletive pairs
objectives:
  - "Learner knows common prefix patterns for aspect"
  - "Learner recognizes suffix changes in aspect pairs"
  - "Learner can predict aspect from verb form"
  - "Learner knows suppletive pairs"
vocabulary_count: 30
---
# Warm-up

Now that you understand aspect conceptually and can use it in past and future tenses, let's go deeper into HOW aspect pairs are formed. Understanding the patterns will help you predict aspects for new verbs!

**What you'll learn:**
- Common prefixes that create perfective verbs
- Suffix changes in aspect pairs
- Suppletive pairs (verbs with completely different roots)
- Strategies for learning new aspect pairs

---

# Presentation

## Prefixation: The Most Common Pattern

Most perfective verbs are formed by adding a prefix to the imperfective base. Here are the main patterns:

### Prefix: про- (through, completion)

| Imperfective | Perfective | Meaning |
|--------------|------------|---------|
| читати | **про**читати | to read (through) |
| слухати | **про**слухати | to listen (to completion) |
| лежати | **про**лежати | to lie (for a duration) |

**Meaning:** "про-" often suggests going through something completely.

### Prefix: на- (writing/filling)

| Imperfective | Perfective | Meaning |
|--------------|------------|---------|
| писати | **на**писати | to write |
| малювати | **на**малювати | to draw/paint |
| вчити | **на**вчити | to teach (to completion) |

**Meaning:** "на-" often relates to creating something on a surface.

### Prefix: з-/с- (completion)

| Imperfective | Perfective | Meaning |
|--------------|------------|---------|
| робити | **з**робити | to do/make |
| їсти | **з**'їсти | to eat up |
| ламати | **з**ламати | to break |

**Meaning:** "з-" is a general completion marker.

> 💡 **Spelling Rule**
>
> The prefix "з-" becomes "с-" before voiceless consonants: сказати (not "зказати"), створити (not "зтворити").

### Prefix: по- (beginning or brief action)

| Imperfective | Perfective | Meaning |
|--------------|------------|---------|
| бачити | **по**бачити | to see (catch sight of) |
| чути | **по**чути | to hear (catch sound of) |
| любити | **по**любити | to fall in love |

**Meaning:** "по-" can mark the start of a state or a brief action.

### Prefix: ви- (out, mastery)

| Imperfective | Perfective | Meaning |
|--------------|------------|---------|
| вчити | **ви**вчити | to learn/master |
| пити | **ви**пити | to drink up |
| ходити | **ви**йти | to go out |

**Meaning:** "ви-" often means "out" or complete mastery.

> 📜 **Pattern Recognition Tip**
>
> When you see a verb with a prefix, it's usually perfective. The underlying stem (without prefix) is usually imperfective. Exception: some prefixed verbs have their own imperfective forms!

## Suffixation Patterns

Some aspect pairs change their suffix, typically involving -ва- or -ува-:

| Imperfective | Perfective | Pattern |
|--------------|------------|---------|
| давати | дати | -ва- removal |
| вставати | встати | -ва- removal |
| відпочивати | відпочити | -ва- removal |
| показувати | показати | -ува- removal |

**The pattern:** Imperfective has -ва- or -ува-; perfective doesn't.

> [!tip] Easy Recognition
> If a verb ends in -вати or -увати, it's almost always imperfective. Remove the suffix, and you often get the perfective!

## Suppletive Pairs (Different Roots)

Some aspect pairs use completely different words. You must memorize these:

| Imperfective | Perfective | Meaning |
|--------------|------------|---------|
| говорити | сказати | to say/speak |
| брати | взяти | to take |
| класти | покласти | to put |
| шукати | знайти | to look for / find |

**Meaning:** These pairs often have slightly different nuances — говорити (process of speaking) vs. сказати (saying something specific).

---

# Cultural Insight

## Aspect and Word Formation

Understanding aspect patterns helps you learn vocabulary faster. When you encounter a new verb, ask:

1. **Does it have a prefix?** → Probably perfective
2. **Does it have -ва-/-ува-?** → Probably imperfective
3. **What's the basic meaning?** → Look for the pair!

**Example:** You see "показувати" for the first time.
- Has -ува- suffix → imperfective
- Remove suffix → показ- → perfective: показати

> 🔍 **Prefixes Can Change Meaning!**
>
> Be careful: prefixes don't just perfectivize — they can change meaning entirely:
> - писати (to write) → написати (to write/complete)
> - писати (to write) → переписати (to rewrite)
> - писати (to write) → підписати (to sign)
> Each prefixed verb has its OWN imperfective: переписувати, підписувати.

---

# Practice

## Practice: Identify the Pattern

1. **читати → прочитати** = про- prefix (completion)
2. **писати → написати** = на- prefix
3. **давати → дати** = -ва- removal
4. **бачити → побачити** = по- prefix (moment of seeing)
5. **говорити → сказати** = suppletive (different roots)

---

# Summary

## What You Learned

This module explored how aspect pairs are formed in Ukrainian.

**Key Patterns:**
- **Prefixation:** про-, на-, з-/с-, по-, ви- (most common)
- **Suffixation:** -ва-/-ува- (impf.) → removal (perf.)
- **Suppletive:** Different roots (must memorize)

**Recognition Tips:**
- Prefix usually = perfective
- -ва-/-ува- = imperfective
- Same root, different prefix = related but different verbs

Тепер ви можете розпізнавати аспектні пари! 🔍

# Activities

## match-up: Aspect Pairs

- читати :: прочитати
- писати :: написати
- робити :: зробити
- їсти :: з'їсти
- бачити :: побачити
- чути :: почути
- давати :: дати
- вставати :: встати
- відпочивати :: відпочити
- говорити :: сказати
- брати :: взяти
- показувати :: показати

## fill-in: Identify the Pattern

> What pattern forms this aspect pair?

1. читати → прочитати: ___
   > [!answer] про- prefix
   > [!options] про- prefix | на- prefix | -ва- removal | suppletive

2. писати → написати: ___
   > [!answer] на- prefix
   > [!options] про- prefix | на- prefix | -ва- removal | suppletive

3. давати → дати: ___
   > [!answer] -ва- removal
   > [!options] про- prefix | з- prefix | -ва- removal | suppletive

4. говорити → сказати: ___
   > [!answer] suppletive
   > [!options] по- prefix | с- prefix | -ва- removal | suppletive

5. робити → зробити: ___
   > [!answer] з- prefix
   > [!options] про- prefix | на- prefix | з- prefix | suppletive

6. бачити → побачити: ___
   > [!answer] по- prefix
   > [!options] про- prefix | на- prefix | по- prefix | suppletive

7. вчити → вивчити: ___
   > [!answer] ви- prefix
   > [!options] про- prefix | ви- prefix | -ва- removal | suppletive

8. брати → взяти: ___
   > [!answer] suppletive
   > [!options] в- prefix | ви- prefix | -ва- removal | suppletive

9. вставати → встати: ___
   > [!answer] -ва- removal
   > [!options] про- prefix | з- prefix | -ва- removal | suppletive

10. їсти → з'їсти: ___
    > [!answer] з- prefix
    > [!options] про- prefix | на- prefix | з- prefix | suppletive

11. показувати → показати: ___
    > [!answer] -ува- removal
    > [!options] по- prefix | на- prefix | -ува- removal | suppletive

12. чути → почути: ___
    > [!answer] по- prefix
    > [!options] про- prefix | на- prefix | по- prefix | suppletive

## quiz: Aspect Formation

1. Which prefix means "through/completion"?

- [x] про-
- [ ] на-
- [ ] по-

---

2. Which prefix is common for writing/creating?

- [x] на-
- [ ] про-
- [ ] ви-

---

3. If a verb has -ва- or -ува-, it's usually:

- [x] Imperfective
- [ ] Perfective

---

4. "Говорити / сказати" are:

- [x] Suppletive pairs (different roots)
- [ ] Prefixed pairs
- [ ] Suffixed pairs

---

5. What's the perfective of "давати"?

- [x] дати (remove -ва-)
- [ ] давив
- [ ] подавати

---

6. What's the perfective of "вчити"?

- [x] вивчити (ви- prefix)
- [ ] провчити
- [ ] вчивати

---

7. Prefix "з-" becomes "с-" before:

- [x] Voiceless consonants
- [ ] Vowels
- [ ] All consonants

---

8. "Показувати" is:

- [x] Imperfective (-ува-)
- [ ] Perfective

---

9. Adding different prefixes to одне verb creates:

- [x] Different verbs with different meanings
- [ ] The same verb with same meaning
- [ ] Only aspect change

---

10. "Ви-" often means:

- [x] Out, mastery, completion
- [ ] In, beginning
- [ ] Under

---

11. Number of suppletive pairs is:

- [x] Small, must memorize
- [ ] Large
- [ ] Zero

---

12. When you see a prefixed verb, assume:

- [x] It's probably perfective
- [ ] It's probably imperfective
- [ ] You can't tell

## true-false: Formation Rules

> Decide if each statement is true or false.

- [x] Most perfective verbs are formed with prefixes.
  > Correct!

- [x] Verbs with -ва-/-ува- are usually imperfective.
  > Yes!

- [ ] All aspect pairs use the same root.
  > Suppletive pairs like говорити/сказати use different roots.

- [x] "про-" often means through or completion.
  > Correct! Прочитати = read through.

- [x] Suppletive pairs must be memorized.
  > Yes, no pattern to predict them.

- [ ] "сказати" is imperfective.
  > "Сказати" is perfective. "Говорити" is imperfective.

- [x] "Давати → дати" shows -ва- removal.
  > Correct!

- [x] Prefix changes can change verb meaning.
  > Yes! Писати, написати, переписати, підписати all differ!

- [ ] "Вставати" is perfective.
  > "Вставати" has -ва-, so it's imperfective.

- [x] "По-" can mark moment of perception.
  > Yes! Побачити = to catch sight of.

- [x] Prefixed verbs can have their own imperfective forms.
  > Correct! Переписати → переписувати.

- [ ] All prefixes perfectivize without changing meaning.
  > Different prefixes add different meanings!

## group-sort: Formation Patterns

> Sort these pairs by how the perfective is formed.

### Prefix про-
- читати/прочитати
- слухати/прослухати

### Prefix на-
- писати/написати
- малювати/намалювати

### Prefix з-
- робити/зробити
- їсти/з'їсти

### Prefix по-
- бачити/побачити
- чути/почути

### -ва- removal
- давати/дати
- вставати/встати
- відпочивати/відпочити

### Suppletive
- говорити/сказати
- брати/взяти

## unjumble: Aspect Pattern Sentences

> Put the words in the correct order.

1. про / читати / = / прочитати
   > [!answer] Про- + читати = прочитати.

2. на / писати / = / написати
   > [!answer] На- + писати = написати.

3. з / робити / = / зробити
   > [!answer] З- + робити = зробити.

4. давати / мінус / -ва- / = / дати
   > [!answer] Давати мінус -ва- = дати.

5. говорити / сказати / різні / корені
   > [!answer] Говорити і сказати — різні корені.

6. по / бачити / = / побачити
   > [!answer] По- + бачити = побачити.

7. ви / вчити / = / вивчити
   > [!answer] Ви- + вчити = вивчити.

8. вставати / встати / -ва- / видалення
   > [!answer] Вставати → встати: -ва- видалення.

9. брати / взяти / суплетивна / пара
   > [!answer] Брати/взяти — суплетивна пара.

10. з'їсти / їсти / з- / префікс
    > [!answer] Їсти + з- префікс = з'їсти.

11. відпочивати / відпочити / -ва-
    > [!answer] Відпочивати → відпочити: -ва- removal.

12. показувати / показати / -ува-
    > [!answer] Показувати → показати: -ува- removal.

## error-correction: Find the Perfective

> What's the perfective of each verb?

1. читати → ___
   > [!error] ___
   > [!answer] прочитати
   > [!explanation] Про- prefix for completion.
   > [!options] читати | прочитати | начитати | зачитати

2. писати → ___
   > [!error] ___
   > [!answer] написати
   > [!explanation] На- prefix for writing/creating.
   > [!options] писати | написати | пописати | списати

3. робити → ___
   > [!error] ___
   > [!answer] зробити
   > [!explanation] З- prefix for completion.
   > [!options] робити | зробити | проробити | наробити

4. давати → ___
   > [!error] ___
   > [!answer] дати
   > [!explanation] Remove -ва-.
   > [!options] давати | дати | подавати | надавати

5. говорити → ___
   > [!error] ___
   > [!answer] сказати
   > [!explanation] Suppletive pair — different roots!
   > [!options] говорити | проговорити | сказати | заговорити

6. брати → ___
   > [!error] ___
   > [!answer] взяти
   > [!explanation] Suppletive pair — different roots!
   > [!options] брати | пробрати | взяти | забрати

## cloze: Pattern Recognition

> Identify the aspect formation pattern.

Аспектна пара читати/прочитати використовує [___:1]. Пара писати/написати використовує [___:2]. Пара давати/дати використовує [___:3]. Пара говорити/сказати — це [___:4] пара.

1. про- / на- / -ва- removal / suppletive
> [!answer] про-

2. про- / на- / -ва- removal / suppletive
> [!answer] на-

3. про- / на- / -ва- removal / suppletive
> [!answer] -ва- removal

4. prefixed / suffixed / suppletive
> [!answer] suppletive

---

Префікс [___:1] додає значення "завершення" чи "через". Префікс [___:2] часто пов'язаний з писанням. Суфікс [___:3] зазвичай є ознакою недоконаного виду. Дієслова на кшталт брати/взяти — це [___:4] пари.

1. про- / на- / по- / ви-
> [!answer] про-

2. про- / на- / по- / ви-
> [!answer] на-

3. -ти / -ва- / -ати / -ити
> [!answer] -ва-

4. prefixed / suffixed / suppletive
> [!answer] suppletive

## dialogue-reorder: Learning Aspect

- Як утворити доконаний вид від "читати"?
- Додай префікс "про-" і отримаєш "прочитати."
- А від "давати"?
- Тут інакше — видали суфікс "-ва-" і отримаєш "дати."
- А "говорити"?
- Це суплетивна пара — доконаний вид "сказати" має інший корінь!

> [!answer]
> 1. Як утворити доконаний вид від "читати"?
> 2. Додай префікс "про-" і отримаєш "прочитати."
> 3. А від "давати"?
> 4. Тут інакше — видали суфікс "-ва-" і отримаєш "дати."
> 5. А "говорити"?
> 6. Це суплетивна пара — доконаний вид "сказати" має інший корінь!

---

- Чому "написати" і "переписати" різні?
- Бо різні префікси додають різні значення.
- Яке значення "пере-"?
- "Пере-" означає "знову" або "через."
- Отже, "переписати" — написати знову?
- Так, точно!

> [!answer]
> 1. Чому "написати" і "переписати" різні?
> 2. Бо різні префікси додають різні значення.
> 3. Яке значення "пере-"?
> 4. "Пере-" означає "знову" або "через."
> 5. Отже, "переписати" — написати знову?
> 6. Так, точно!

## mark-the-words: Identify Perfective Verbs

Click on PERFECTIVE verbs only.

Я [прочитав](correct) книгу. Потім я [написав](correct) рецензію. Він [сказав](correct) правду. Вона довго говорила.

---

Ми [вивчили](correct) урок. Вона [встала](correct) рано. Він давав книгу, а потім [дав](correct).

---

Вони [побачили](correct) море! Він [почув](correct) шум. Я [взяв](correct) книгу і [пішов](correct).

## translate: English to Ukrainian

I read through the book. (finished)

- [x] Я прочитав книгу.
- [ ] Я читав книгу.

---

She gave the book. (completed)

- [x] Вона дала книгу.
- [ ] Вона давала книгу.

---

He said the truth.

- [x] Він сказав правду.
- [ ] Він говорив правду.

---

I learned the lesson. (mastered)

- [x] Я вивчив урок.
- [ ] Я вчив урок.

---

She stood up.

- [x] Вона встала.
- [ ] Вона вставала.

---

He took the book.

- [x] Він узяв книгу.
- [ ] Він брав книгу.

---

We saw the sea!

- [x] Ми побачили море!
- [ ] Ми бачили море!

---

She rested. (completed)

- [x] Вона відпочила.
- [ ] Вона відпочивала.

---

He showed the photo.

- [x] Він показав фото.
- [ ] Він показував фото.

---

They heard the news.

- [x] Вони почули новини.
- [ ] Вони чули новини.

---

I ate it all.

- [x] Я з'їв усе.
- [ ] Я їв усе.

---

She drank the coffee.

- [x] Вона випила каву.
- [ ] Вона пила каву.

---

# Vocabulary

| Word | IPA | English | POS | Gender | Note |
|------|-----|---------|-----|--------|------|
| відпочивати | /ʋidpɔˈt͡ʃɪʋɑtɪ/ | to rest | verb | — | imperfective |
| відпочити | /ʋidpɔˈt͡ʃɪtɪ/ | to rest | verb | — | perfective |
| вставати | /ʋstɑˈʋɑtɪ/ | to get up | verb | — | imperfective |
| встати | /ˈʋstɑtɪ/ | to get up | verb | — | perfective |
| одягатися | /ɔdʲɑˈɦɑtɪsʲɑ/ | to dress | verb | — | imperfective |
| одягнутися | /ɔdʲɑɦˈnutɪsʲɑ/ | to dress | verb | — | perfective |
| сідати | /siˈdɑtɪ/ | to sit down | verb | — | imperfective |
| сісти | /ˈsistɪ/ | to sit down | verb | — | perfective |
| лягати | /lʲɑˈɦɑtɪ/ | to lie down | verb | — | imperfective |
| лягти | /ˈlʲɑɦtɪ/ | to lie down | verb | — | perfective |
| відкривати | /ʋidkrɪˈʋɑtɪ/ | to open | verb | — | imperfective |
| відкрити | /ʋidˈkrɪtɪ/ | to open | verb | — | perfective |
| закривати | /zɑkrɪˈʋɑtɪ/ | to close | verb | — | imperfective |
| закрити | /zɑˈkrɪtɪ/ | to close | verb | — | perfective |
| вмикати | /ʋmɪˈkɑtɪ/ | to turn on | verb | — | imperfective |
| увімкнути | /uʋimkˈnutɪ/ | to turn on | verb | — | perfective |
| вимикати | /ʋɪmɪˈkɑtɪ/ | to turn off | verb | — | imperfective |
| вимкнути | /ˈʋɪmknutɪ/ | to turn off | verb | — | perfective |
| починати | /pɔt͡ʃɪˈnɑtɪ/ | to begin | verb | — | imperfective |
| почати | /pɔˈt͡ʃɑtɪ/ | to begin | verb | — | perfective |
| закінчувати | /zɑkint͡ʃuˈʋɑtɪ/ | to finish | verb | — | imperfective |
| закінчити | /zɑˈkint͡ʃɪtɪ/ | to finish | verb | — | perfective |
| отримувати | /ɔtrɪˈmuʋɑtɪ/ | to receive | verb | — | imperfective |
| отримати | /ɔˈtrɪmɑtɪ/ | to receive | verb | — | perfective |
| посилати | /pɔsɪˈlɑtɪ/ | to send | verb | — | imperfective |
| послати | /pɔˈslɑtɪ/ | to send | verb | — | perfective |
| вибирати | /ʋɪbɪˈrɑtɪ/ | to choose | verb | — | imperfective |
| вибрати | /ˈʋɪbrɑtɪ/ | to choose | verb | — | perfective |
| вирішувати | /ʋɪriˈʃuʋɑtɪ/ | to decide | verb | — | imperfective |
| вирішити | /ʋɪˈriʃɪtɪ/ | to decide | verb | — | perfective |

---
