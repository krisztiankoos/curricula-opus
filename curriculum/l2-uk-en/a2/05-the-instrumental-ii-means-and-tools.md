---
module: a2-05
title: "The Instrumental II — Means & Tools"
subtitle: "By Bus, With a Pen"
version: "1.0"
phase: "A2.1"
pedagogy: "PPP"
duration: "60 min"
transliteration: none
tags:
  - grammar
  - cases
  - instrumental
  - transport
  - tools
grammar:
  - instrumental without preposition
  - means of transport
  - tools and instruments
objectives:
  - "Learner can use instrumental without preposition for means of transport"
  - "Learner can use instrumental for tools and instruments"
  - "Learner can express manner using instrumental case"
  - "Learner can distinguish between з + Instrumental and bare Instrumental"
vocabulary_count: 28
---

## Introduction

Module 4 introduced the Instrumental case with the preposition **з** for accompaniment: з другом (with a friend), з сестрою (with sister). Now it's time to learn the Instrumental case **without a preposition** — the true "instrumental" use of this case!

**What you'll learn:**
- **Bare Instrumental for transport:** автобусом (by bus), поїздом (by train), літаком (by plane)
- **Bare Instrumental for tools:** ручкою (with a pen), ножем (with a knife), ложкою (with a spoon)
- **Bare Instrumental for body parts:** рукою (with hand), головою (with head)
- **Bare Instrumental for manner:** голосом (with voice), пошепки (in a whisper)

**The key distinction:**
- **З + Instrumental** = accompaniment (with someone/something as a companion)
- **Bare Instrumental** = means, tools, manner (by what means, with what tool, in what way)

**Why this matters:**
English uses "by" for transport (by bus) and "with" for tools (with a pen), but Ukrainian uses the same grammatical case for both — the Instrumental without a preposition. This is the Instrumental in its most literal sense: the case of instruments and means.

## Warm-up

У попередньому модулі ви вивчили орудний відмінок з прийменником **з** для супроводу: з другом, з сестрою. Тепер час дізнатися про орудний відмінок **без прийменника**!

When you want to say "by bus," "with a pen," or "in a loud voice," Ukrainian uses the instrumental case WITHOUT any preposition. "I'm going **by bus**" = Я їду **автобусом**. "She writes **with a pen**" = Вона пише **ручкою**.

This is the "instrumental" in its truest sense — the case of instruments and means!

> [!resources] 🔗 External Resources
>
> **Grammar Guide:** [Ukrainian Cases Overview](https://www.ukrainianlessons.com/intro-cases/) — All 7 cases explained
>
> **Interactive:** [Talk Ukrainian - Cases](https://talkukrainian.com/grammatical-cases/) — Practice exercises

---

## Presentation

## The Bare Instrumental: No Preposition Needed

У попередньому модулі ви вивчили орудний відмінок із прийменником **з** для супроводу. Тепер ми розглянемо орудний відмінок **без прийменника**.

The instrumental case without a preposition answers:
- **Чим?** — By what? | With what? (means, tool)
- **Як?** — How? (manner)

This is the instrumental in its most literal sense — the case of instruments, tools, and the means by which actions are performed. Think of it as the "by" case: you travel **by bus**, write **with a pen**, and speak **in a voice**.

| With preposition | Without preposition |
|------------------|---------------------|
| йти **з** другом (with friend) | їхати **автобусом** (by bus) |
| кава **з** молоком (with milk) | писати **ручкою** (with pen) |
| жити **з** батьками (with parents) | говорити **голосом** (in voice) |

## Means of Transport

Орудний відмінок без прийменника часто використовується для транспорту. "By bus" = автобусом, "by train" = поїздом. No preposition needed in Ukrainian:

When describing how you travel, use instrumental without a preposition:

| Transport | Nominative | Instrumental | Example |
|-----------|------------|--------------|---------|
| bus | автобус | автобусом | їхати автобусом |
| tram | трамвай | трамваєм | їхати трамваєм |
| trolleybus | тролейбус | тролейбусом | їхати тролейбусом |
| metro | метро | метро | їхати метро |
| car | машина | машиною | їхати машиною |
| taxi | таксі | таксі | їхати таксі |
| train | поїзд | поїздом | їхати поїздом |
| plane | літак | літаком | летіти літаком |

Я їду на роботу автобусом. (I go to work by bus.)
Вона їде в центр трамваєм. (She's going downtown by tram.)
Ми летимо до Львова літаком. (We're flying to Lviv by plane.)

> [!tip] Метро і Таксі — Indeclinable Nouns
> These are **indeclinable** — they don't change form in any case! Їхати метро, їхати таксі.
>
> **Why?** Both words are borrowed from other languages (French/Italian) and end in vowels that don't fit Ukrainian declension patterns. This actually makes them easier to use — no endings to memorize!
>
> **Common indeclinable nouns:** метро, таксі, кіно, кафе, меню, радіо — all borrowed words ending in vowels that don't decline.

### На + Accusative vs. Instrumental

Sometimes you'll hear "на автобусі" (on the bus) — this is locative and means physically being ON the vehicle. The instrumental describes the mode of transport:

| Expression | Meaning |
|------------|---------|
| їхати автобусом | to travel by bus (means) |
| сидіти на автобусі | to sit on (top of) the bus |
| їхати в автобусі | to ride in the bus (inside) |

> [!example] У Києві — Commute Conversation
> This dialogue shows Ukrainians discussing their daily commute. Notice how metro and taxi don't change form (indeclinable), while автобусом is regular instrumental.
>
> — Як ти добираєшся до роботи? (How do you get to work?)
> — Зазвичай їду метро. А коли метро закрите, їду таксі. (Usually by metro. When metro is closed, by taxi.)
> — А я їду автобусом. Автобус зупиняється біля мого будинку. (I go by bus. The bus stops near my building.)
> — Це зручно! Мені треба йти до станції метро пішки. (That's convenient! I have to walk to the metro station.)

## Tools and Instruments

The second major use of the bare instrumental is for tools. When you describe what tool you use to perform an action, the tool goes in the instrumental case without a preposition.

For example: "I write with a pen" = Я пишу ручкою. "She cuts with a knife" = Вона ріже ножем.

| Tool | Nominative | Instrumental | Example |
|------|------------|--------------|---------|
| pen | ручка | ручкою | писати ручкою |
| pencil | олівець | олівцем | малювати олівцем |
| knife | ніж | ножем | різати ножем |
| fork | виделка | виделкою | їсти виделкою |
| spoon | ложка | ложкою | їсти ложкою |
| scissors | ножиці | ножицями | різати ножицями |
| needle | голка | голкою | шити голкою |
| hammer | молоток | молотком | забивати молотком |
| brush | пензель | пензлем | малювати пензлем |

Я пишу листа ручкою. (I'm writing a letter with a pen.)
Вона ріже хліб ножем. (She's cutting bread with a knife.)
Ми їмо суп ложкою. (We eat soup with a spoon.)

> [!info] Ножиці — Plural Only (Pluralia Tantum)
> The word **ножиці** exists only in plural form (like English "scissors"), so its instrumental is ножицями.
>
> **Other plural-only nouns in Ukrainian:**
> - окуляри → окулярами
> - штани → штанами
> - двері → дверима
> - гроші → грошима
>
> **English comparison:** English also has plural-only nouns like scissors, glasses, pants, and jeans. But Ukrainian has many more!

### Body Parts as Instruments

Body parts can also be used as instruments: wave with your hand, nod with your head, kick with your feet:

| Body Part | Nominative | Instrumental | Example |
|-----------|------------|--------------|---------|
| hand | рука | рукою/руками | махати рукою |
| foot/leg | нога | ногою/ногами | бити ногою |
| head | голова | головою | кивати головою |
| eyes | очі | очима | дивитися очима |

Він махає рукою. (He's waving with his hand.)
Вона киває головою. (She's nodding with her head.)
Дитина б'є ногами. (The child is kicking with its legs.)

> [!warning] Очі — Irregular Instrumental
> The word **очі** has an irregular instrumental: **очима** instead of the expected *очами*.
>
> **Why?** This is a historical remnant from Old Ukrainian. The form очима preserves an ancient dual number pattern (for pairs of body parts).
>
> **Other irregular body part instrumentals:**
> - вухо → вухами is regular today, BUT historically вушима
> - плече → плечами is regular today, BUT historically плечима
>
> **Good news:** Очима is the main irregular one you need to remember. Most body parts follow regular patterns.

## Communication and Technology

Modern technology also uses the instrumental case. You work with a computer, send by email, and call by phone — all using bare instrumental forms:

| Device | Nominative | Instrumental | Example |
|--------|------------|--------------|---------|
| computer | комп'ютер | комп'ютером | працювати комп'ютером |
| phone | телефон | телефоном | дзвонити телефоном |
| email | електронна пошта | електронною поштою | надсилати електронною поштою |

Я працюю комп'ютером щодня. (I work with a computer every day.)
Вона надсилає документи електронною поштою. (She sends documents by email.)

> [!tip] Телефоном vs. По телефону — Both Are Correct
> Both constructions mean "by phone" and are interchangeable in modern Ukrainian:
> - дзвонити **телефоном** = bare instrumental (classic pattern)
> - дзвонити **по телефону** = preposition + locative (also common)
>
> **Similar alternations:**
> - надсилати поштою | надсилати по пошті (by mail)
> - передавати радіо | передавати по радіо (by radio)
>
> **Which to use?** Both are correct. The bare instrumental is considered more traditional, while "по + locative" is common in everyday speech.

---

## Cultural Insight

## Manner and Voice

The instrumental case also expresses manner — how an action is performed. This is especially common with voice: you speak in a quiet voice, shout loudly, or whisper:

| Manner | Nominative | Instrumental | Example |
|--------|------------|--------------|---------|
| voice | голос | голосом | говорити тихим голосом |
| whisper | пошепки | пошепки | говорити пошепки |
| loud | голосно | голосом | кричати голосом |

Він говорить тихим голосом. (He speaks in a quiet voice.)
Вона каже це пошепки. (She says this in a whisper.)
Дітей не варто кричати голосом. (One shouldn't shout at children.)

> [!example] У бібліотеці в Одесі — Library Etiquette
> This dialogue demonstrates manner expressions with instrumental. "Тихим голосом" (in a quiet voice) combines an adjective with instrumental noun.
>
> — Пробачте, тут треба говорити тихим голосом. (Excuse me, you need to speak in a quiet voice here.)
> — Вибачте! Я не помітив. (Sorry! I didn't notice.)
> — Будь ласка, розмовляйте пошепки. (Please speak in a whisper.)
> — Звичайно. Дякую за нагадування. (Of course. Thanks for the reminder.)

## Ukrainian Transport Culture

Ukrainian cities have extensive public transport systems. Here are interesting facts about transport in major Ukrainian cities:

| City | Famous For |
|------|------------|
| Київ | Найглибше метро у світі! |
| Львів | Історичні трамваї |
| Одеса | Трамвай і тролейбус |
| Харків | Друге метро в Україні |

> [!tip] Маршрутка — The Ukrainian Minibus
> The **маршрутка** (marshrutka) is a uniquely post-Soviet phenomenon — a small minibus that runs on fixed routes throughout Ukrainian cities.
>
> **Key facts:**
> - Faster than regular buses (fewer stops)
> - You pay the driver directly when boarding
> - Shout "На зупинці!" (At the stop!) when you want to get off
> - Standing passengers hold on tight — the ride can be bumpy!
>
> **Instrumental:** Їхати **маршруткою** (by marshrutka)
>
> **Cultural note:** Marshrutkas are quintessentially Ukrainian/post-Soviet. You won't find them in Western Europe. They're efficient but crowded, especially during rush hour!

> [!did-you-know] 💡 Did You Know
> The Kyiv Metro has some of the deepest stations in the world! Arsenalna station is 105.5 meters below ground — the deepest metro station on Earth. This depth was originally designed for civil defense purposes during the Soviet era. When you say "Я їду метро до Арсенальної," you're traveling to a piece of engineering history!

Я їду маршруткою на базар. (I'm going to the market by minibus.)
У Києві зручно їздити метро. (In Kyiv it's convenient to travel by metro.)

> [!example] Подорож до Карпат — Travel Story
> This dialogue shows multiple transport modes in one trip. Notice the pattern: always bare instrumental for the transport (поїздом, автобусом), never with preposition "з".
>
> — Як ви добиралися до Карпат? (How did you get to the Carpathians?)
> — Спочатку ми їхали поїздом до Івано-Франківська. (First we went by train to Ivano-Frankivsk.)
> — А далі? (And then?)
> — Потім їхали автобусом до Яремче. А в гори ми йшли пішки! (Then by bus to Yaremche. And into the mountains we walked!)
> — Звучить як пригода! (Sounds like an adventure!)

---

## Practice

Now let's practice using the bare Instrumental for transport, tools, and manner. The key skill is distinguishing between accompaniment (з + Instrumental) and means/tools (bare Instrumental).

**Quick reference:**
- **Accompaniment (з + Instr):** з другом (with friend), з молоком (with milk)
- **Transport (bare Instr):** автобусом (by bus), поїздом (by train)
- **Tools (bare Instr):** ручкою (with pen), ножем (with knife)
- **Manner (bare Instr):** голосом (with voice), пошепки (in whisper)

## Choose: With or Without Preposition?

| Sentence | Preposition needed? |
|----------|---------------------|
| Я йду __ другом | з — with someone |
| Я їду __ автобусом | — no preposition |
| Вона пише __ ручкою | — no preposition |
| Кава __ молоком | з — with ingredient |
| Він говорить тихим __ голосом | — no preposition |

## Form the Instrumental

| Nominative | Instrumental |
|------------|--------------|
| автобус | автобусом |
| ручка | ручкою |
| ніж | ножем |
| ложка | ложкою |
| поїзд | поїздом |
| голова | головою |

## Mini-Dialogues

The following dialogues show bare Instrumental in natural conversations about transport, tools, and everyday situations.

**Dialogue 1: На зупинці (At the Bus Stop)**

Getting directions — notice both автобусом and трамваєм are bare Instrumental (no preposition).

**На зупинці:**
— Вибачте, як дістатися до центру?
— Можна їхати автобусом або трамваєм.
— А що швидше?
— Трамваєм швидше, але автобусом зручніше.

**Dialogue 2: На кухні (In the Kitchen)**

Discussing tools — чим (with what) asks about Instrumental; ножем and ножицями are the answers.

**На кухні:**
— Чим ти ріжеш помідори?
— Ножем, звичайно!
— А я волію різати ножицями.
— Ножицями? Це дивно!

**Dialogue 3: В офісі (At the Office)**

Modern communication — електронною поштою (by email) and голосом (by voice) are both bare Instrumental.

**В офісі:**
— Як ви надсилаєте документи?
— Зазвичай електронною поштою.
— А якщо терміново?
— Тоді телефоную і пояснюю голосом.

---

## Reading Practice: Один день у Києві

This text describes a typical day in Kyiv, filled with bare Instrumental examples: transport (метро, автобусом, трамваєм), tools (ложкою, ножем, виделкою), and technology (комп'ютером, електронною поштою).

**Reading strategy:** As you read, highlight every bare Instrumental (without preposition). Then compare with the answers to verify you've found them all.

**Vocabulary preview:**
- прокидатися — to wake up
- дістатися — to get to (a place)
- терміновий — urgent
- готувати — to cook/prepare

> Ранок. Я прокидаюся о сьомій годині. Снідаю: їм кашу ложкою, п'ю каву. Потім їду на роботу.
>
> До станції метро йду пішки — це десять хвилин. Потім їду метро до центру. У метро читаю книгу.
>
> На роботі працюю комп'ютером. Пишу листи колегам електронною поштою. Якщо щось термінове — дзвоню телефоном.
>
> Увечері їду додому. Іноді їду автобусом, іноді — трамваєм. Коли йде дощ, їду таксі.
>
> Вдома готую вечерю. Ріжу овочі ножем. Їм виделкою і ложкою. Потім дивлюся фільм і йду спати.

**Questions:**
1. Чим їсть кашу? — Ложкою.
2. Чим їде на роботу? — Метро.
3. Чим працює на роботі? — Комп'ютером.
4. Чим надсилає листи? — Електронною поштою.
5. Чим ріже овочі? — Ножем.

---

## Підсумок

У цьому модулі ви навчилися:

- **Bare instrumental for transport**: автобусом, трамваєм, поїздом, літаком, машиною
  - Я їду автобусом. (NOT з автобусом)
  - Indeclinable: метро, таксі

- **Bare instrumental for tools**: ручкою, олівцем, ножем, ложкою, виделкою
  - Вона пише ручкою. (NOT з ручкою)

- **Bare instrumental for body parts**: рукою, ногою, головою, очима
  - Він махає рукою. (NOT з рукою)

- **Bare instrumental for manner**: голосом, пошепки
  - Він говорить тихим голосом. (NOT з голосом)

- **Key distinction**:
  - **з + Instrumental** = accompaniment (з другом, з сестрою)
  - **Bare Instrumental** = means, tools, manner (автобусом, ножем, голосом)

Тепер ви готові! Ви можете розповісти, чим їдете на роботу і чим пишете. 🚌✏️🗣️

## Activities

The following activities will help you master the bare Instrumental case. Remember: no preposition is needed for transport, tools, body parts, or manner!

**Success tip:** If you're expressing the MEANS by which something is done (transport, tool, method), use bare Instrumental. If you're expressing ACCOMPANIMENT (with a person, with an ingredient), use з + Instrumental.

## match-up: Transport and Tools

- автобусом :: by bus
- поїздом :: by train
- ручкою :: with a pen
- ножем :: with a knife
- літаком :: by plane
- ложкою :: with a spoon
- машиною :: by car
- олівцем :: with a pencil
- трамваєм :: by tram
- виделкою :: with a fork
- таксі :: by taxi
- голосом :: with voice

## fill-in: Instrumental Without Preposition

> Complete each sentence with the correct instrumental form (no preposition).

1. Ми їдемо в аеропорт ___.
   > [!answer] таксі
   > [!options] таксі | з таксі | на таксі

2. Вона пише листа ___.
   > [!answer] ручкою
   > [!options] ручкою | ручка | з ручкою

3. Він ріже хліб ___.
   > [!answer] ножем
   > [!options] ножем | ніж | з ножем

4. Ми їмо суп ___.
   > [!answer] ложкою
   > [!options] ложкою | ложка | з ложкою

5. Вона летить до Парижа ___.
   > [!answer] літаком
   > [!options] літаком | літак | з літаком

6. Він говорить тихим ___.
   > [!answer] голосом
   > [!options] голосом | голос | з голосом

7. Я працюю ___ щодня.
   > [!answer] комп'ютером
   > [!options] комп'ютером | комп'ютер | з комп'ютером

8. Діти малюють ___.
   > [!answer] олівцем
   > [!options] олівцем | олівець | з олівцем

9. Вона махає ___.
   > [!answer] рукою
   > [!options] рукою | рука | з рукою

10. Ми їдемо в центр ___.
    > [!answer] трамваєм
    > [!options] трамваєм | трамвай | з трамваєм

11. Я надсилаю документи ___.
    > [!answer] електронною поштою
    > [!options] електронною поштою | електронна пошта | з електронною поштою

12. Вона киває ___.
    > [!answer] головою
    > [!options] головою | голова | з головою

## quiz: Instrumental Uses

1. Як сказати "by bus"?

- [ ] з автобусом
- [x] автобусом
- [ ] на автобус

---

2. Яке речення правильне?

- [x] Я пишу ручкою.
- [ ] Я пишу з ручкою.
- [ ] Я пишу ручка.

---

3. Коли використовуємо орудний БЕЗ прийменника?

- [x] Для засобу пересування та інструментів
- [ ] Для супроводу (з ким?)
- [ ] Для місця (де?)

---

4. Як сказати "with a knife"?

- [x] ножем
- [ ] з ножем
- [ ] ножа

---

5. Яке речення правильне?

- [x] Він говорить тихим голосом.
- [ ] Він говорить з тихим голосом.
- [ ] Він говорить тихий голос.

---

6. Як сказати "by train"?

- [x] поїздом
- [ ] з поїздом
- [ ] на поїзд

---

7. "Метро" в орудному відмінку...

- [x] не змінюється
- [ ] стає "метром"
- [ ] стає "метрою"

---

8. Як сказати "with eyes"?

- [x] очима
- [ ] очами
- [ ] з очами

---

9. Яке речення правильне?

- [x] Вона ріже хліб ножем.
- [ ] Вона ріже хліб з ножем.
- [ ] Вона ріже хліб ніж.

---

10. Як сказати "by email"?

- [x] електронною поштою
- [ ] з електронною поштою
- [ ] електронна пошта

---

11. "Ножиці" в орудному відмінку...

- [x] ножицями
- [ ] ножицею
- [ ] ножицям

---

12. Як сказати "in a whisper"?

- [x] пошепки
- [ ] з пошепки
- [ ] пошепкою

## true-false: Instrumental Rules
> Decide if each statement is true or false.

- [ ] For means of transport, we use "з + Instrumental."
  > For transport, we use bare instrumental: їхати автобусом.

- [x] "Метро" and "таксі" don't change form in the instrumental.
  > Correct! They are indeclinable: їхати метро, їхати таксі.

- [x] "Очима" is the instrumental of "очі."
  > Yes! Очі has an irregular instrumental: очима.

- [ ] We always use a preposition with the instrumental case.
  > The instrumental can be used without a preposition for means, tools, and manner.

- [x] "Ручкою" means "with a pen" (tool).
  > Correct! Писати ручкою = to write with a pen.

- [ ] "Їхати автобусом" and "їхати на автобусі" mean the same thing.
  > Автобусом = by bus. На автобусі = on the bus.

- [x] Body parts can be used in the instrumental to describe actions.
  > Yes! Махати рукою, кивати головою, бити ногами.

- [x] "Голосом" is used to express manner of speaking.
  > Correct! Говорити тихим голосом = to speak in a quiet voice.

- [ ] "Ножиці" has a singular form.
  > Ножиці exists only in plural. Its instrumental is ножицями.

- [x] "Електронною поштою" means "by email."
  > Correct! Надсилати електронною поштою = to send by email.

- [ ] For accompaniment, we use bare instrumental.
  > For accompaniment, we use з + instrumental: з другом, з сестрою.

- [x] "Пошепки" is an adverb, not a noun in instrumental.
  > Correct! Пошепки is an adverb meaning "in a whisper."

## group-sort: Transport vs. Tools
> Sort these words into the correct category.

### Transport
- автобусом
- трамваєм
- поїздом
- літаком
- машиною
- метро

### Tools
- ручкою
- олівцем
- ножем
- ложкою
- виделкою
- ножицями

## unjumble: Instrumental Sentences
> Put the words in the correct order.

1. вона | їде | до | центру | трамваєм
   > [!answer] Вона їде до центру трамваєм
   > (She goes to the center by tram.)

2. вона | пише | листа | ручкою
   > [!answer] Вона пише листа ручкою
   > (She writes a letter with a pen.)

3. він | ріже | хліб | ножем
   > [!answer] Він ріже хліб ножем
   > (He cuts bread with a knife.)

4. ми | їмо | суп | ложкою
   > [!answer] Ми їмо суп ложкою
   > (We eat soup with a spoon.)

5. вона | летить | до | Парижа | літаком
   > [!answer] Вона летить до Парижа літаком
   > (She flies to Paris by plane.)

6. він | говорить | тихим | голосом
   > [!answer] Він говорить тихим голосом
   > (He speaks in a quiet voice.)

## error-correction: Fix the Instrumental
> Each sentence has ONE error. Find and correct it.

1. Я їду на роботу з автобусом.
   > [!error] з автобусом
   > [!answer] автобусом
   > [!options] з автобусом | автобусом | автобус | на автобус
   > [!explanation] For means of transport, use bare Instrumental WITHOUT preposition. "By bus" = автобусом, not з автобусом.

2. Вона пише листа з ручкою.
   > [!error] з ручкою
   > [!answer] ручкою
   > [!options] з ручкою | ручкою | ручка | ручку
   > [!explanation] For tools, use bare Instrumental WITHOUT preposition. "With a pen" = ручкою, not з ручкою.

3. Він ріже хліб з ніж.
   > [!error] з ніж
   > [!answer] ножем
   > [!options] з ніж | ножем | ніж | ножа
   > [!explanation] Tools use bare Instrumental: ніж → ножем. No preposition needed, and the noun must be in Instrumental case.

4. Ми їмо суп ложка.
   > [!error] ложка
   > [!answer] ложкою
   > [!options] ложка | ложкою | ложки | ложку
   > [!explanation] Tools require Instrumental case. Feminine nouns ending in -а take -ою: ложка → ложкою.

5. Вона летить до Парижа з літаком.
   > [!error] з літаком
   > [!answer] літаком
   > [!options] з літаком | літаком | літак | літака
   > [!explanation] For means of transport, use bare Instrumental WITHOUT preposition. "By plane" = літаком, not з літаком.

6. Він говорить тихим з голосом.
   > [!error] з голосом
   > [!answer] голосом
   > [!options] з голосом | голосом | голос | голосу
   > [!explanation] Manner expressions use bare Instrumental. "In a quiet voice" = тихим голосом, not з голосом.

## cloze: Complete the Story
> Fill in the blanks with the correct words.

Вранці я йду на зупинку і їду [___:1] на роботу. На роботі працюю [___:2] цілий день. Пишу листи [___:3] і надсилаю їх [___:4]. Увечері їду додому [___:5] — це швидше.

1. автобусом | з автобусом | автобус
   > [!answer] автобусом

2. комп'ютером | з комп'ютером | комп'ютер
   > [!answer] комп'ютером

3. ручкою | з ручкою | ручка
   > [!answer] ручкою

4. електронною поштою | з електронною поштою | електронна пошта
   > [!answer] електронною поштою

5. метро | з метро | на метро
   > [!answer] метро

---

На кухні мама готує обід. Вона ріже м'ясо [___:1]. Салат ріже [___:2]. Потім ми сідаємо за стіл і їмо: суп [___:3], а другу страву [___:4]. Тато розповідає історію тихим [___:5].

1. ножем | з ножем | ніж
   > [!answer] ножем

2. ножицями | з ножицями | ножиці
   > [!answer] ножицями

3. ложкою | з ложкою | ложка
   > [!answer] ложкою

4. виделкою | з виделкою | виделка
   > [!answer] виделкою

5. голосом | з голосом | голос
   > [!answer] голосом

## dialogue-reorder: Asking Directions

- Дякую! До побачення!
- Добрий день! Як дістатися до центру?
- Автобусом швидше, але метро зручніше.
- Можна їхати автобусом або метро.
- А що швидше?
- Тоді поїду метро. Дякую!

> [!answer]
> 1. Добрий день! Як дістатися до центру?
> 2. Можна їхати автобусом або метро.
> 3. А що швидше?
> 4. Автобусом швидше, але метро зручніше.
> 5. Тоді поїду метро. Дякую!
> 6. Дякую! До побачення!

---

- Звичайно. Дай мені ніж.
- Ось. Тепер ріжу ножем.
- Чим ти ріжеш помідори?
- Можеш нарізати сир ножем?
- Ручкою! Жартую. Ножем, звичайно.
- Готово! Тепер можемо їсти!

> [!answer]
> 1. Чим ти ріжеш помідори?
> 2. Ручкою! Жартую. Ножем, звичайно.
> 3. Можеш нарізати сир ножем?
> 4. Звичайно. Дай мені ніж.
> 5. Ось. Тепер ріжу ножем.
> 6. Готово! Тепер можемо їсти!

## mark-the-words: Find Bare Instrumental

Click on instrumental forms used WITHOUT a preposition (means/tools).

Я їду на роботу [автобусом](correct). Вона пише листа [ручкою](correct). Ми живемо [з батьками](wrong). Він ріже хліб [ножем](correct). Кава [з молоком](wrong) смачна.

---

Вона летить [літаком](correct) до Парижа. Він розмовляє [з другом](wrong). Я працюю [комп'ютером](correct). Діти грають [з котом](wrong). Вона махає [рукою](correct).

---

Ми їмо суп [ложкою](correct). Він гуляє [з собакою](wrong). Вона говорить тихим [голосом](correct). Я їду [поїздом](correct). Він живе [з братом](wrong).

## translate: English to Ukrainian

They travel to Lviv by train.

- [ ] Вони їдуть до Львова з поїздом.
- [x] Вони їдуть до Львова поїздом.
- [ ] Вони їдуть до Львова поїзд.

---

She writes with a pen.

- [ ] Вона пише з ручкою.
- [x] Вона пише ручкою.
- [ ] Вона пише ручка.

---

He cuts bread with a knife.

- [x] Він ріже хліб ножем.
- [ ] Він ріже хліб з ножем.
- [ ] Він ріже хліб ніж.

---

We eat soup with a spoon.

- [x] Ми їмо суп ложкою.
- [ ] Ми їмо суп з ложкою.
- [ ] Ми їмо суп ложка.

---

She flies to Paris by plane.

- [x] Вона летить до Парижа літаком.
- [ ] Вона летить до Парижа з літаком.
- [ ] Вона летить до Парижа літак.

---

He speaks in a quiet voice.

- [x] Він говорить тихим голосом.
- [ ] Він говорить з тихим голосом.
- [ ] Він говорить тихий голос.

---

I work with a computer every day.

- [x] Я працюю комп'ютером щодня.
- [ ] Я працюю з комп'ютером щодня.
- [ ] Я працюю комп'ютер щодня.

---

Children draw with a pencil.

- [x] Діти малюють олівцем.
- [ ] Діти малюють з олівцем.
- [ ] Діти малюють олівець.

---

She waves with her hand.

- [x] Вона махає рукою.
- [ ] Вона махає з рукою.
- [ ] Вона махає рука.

---

We go downtown by tram.

- [x] Ми їдемо в центр трамваєм.
- [ ] Ми їдемо в центр з трамваєм.
- [ ] Ми їдемо в центр трамвай.

---

I send documents by email.

- [x] Я надсилаю документи електронною поштою.
- [ ] Я надсилаю документи з електронною поштою.
- [ ] Я надсилаю документи електронна пошта.

---

She nods with her head.

- [x] Вона киває головою.
- [ ] Вона киває з головою.
- [ ] Вона киває голова.

---

## Vocabulary

| Word | IPA | English | POS | Gender | Note |
|------|-----|---------|-----|--------|------|
| автобусом | /ɑʋˈtobusɔm/ | by bus | adv | — | instrumental of автобус |
| трамваєм | /trɑmˈʋɑjɛm/ | by tram | adv | — | instrumental of трамвай |
| тролейбусом | /trɔˈlejbusɔm/ | by trolleybus | adv | — | instrumental of тролейбус |
| метро | /mɛˈtrɔ/ | metro, by metro | noun | n | indeclinable |
| машиною | /mɑˈʃɪnɔju/ | by car | adv | — | instrumental of машина |
| таксі | /tɑˈksi/ | taxi, by taxi | noun | n | indeclinable |
| поїздом | /ˈpɔjizdɔm/ | by train | adv | — | instrumental of поїзд |
| літаком | /liˈtɑkɔm/ | by plane | adv | — | instrumental of літак |
| ручкою | /ˈrutʃkɔju/ | with a pen | adv | — | instrumental of ручка |
| олівцем | /ɔˈliʋt͡sɛm/ | with a pencil | adv | — | instrumental of олівець |
| ножем | /ˈnɔʒɛm/ | with a knife | adv | — | instrumental of ніж |
| виделкою | /ˈʋɪdɛlkɔju/ | with a fork | adv | — | instrumental of виделка |
| ложкою | /ˈlɔʒkɔju/ | with a spoon | adv | — | instrumental of ложка |
| ножицями | /nɔˈʒɪt͡sʲɑmɪ/ | with scissors | adv | — | instrumental of ножиці |
| голкою | /ˈɦɔlkɔju/ | with a needle | adv | — | instrumental of голка |
| молотком | /mɔlɔtˈkɔm/ | with a hammer | adv | — | instrumental of молоток |
| пензлем | /ˈpɛnzlɛm/ | with a brush | adv | — | instrumental of пензель |
| комп'ютером | /kɔmpjuˈtɛrɔm/ | with a computer | adv | — | instrumental of комп'ютер |
| телефоном | /tɛlɛˈfɔnɔm/ | by phone | adv | — | instrumental of телефон |
| очима | /ɔˈtʃɪmɑ/ | with eyes | adv | — | irregular instr. of очі |
| руками | /ruˈkɑmɪ/ | with hands | adv | — | instrumental pl. of рука |
| ногами | /noˈɦɑmɪ/ | with feet/legs | adv | — | instrumental pl. of нога |
| головою | /ɦɔlɔˈʋɔju/ | with head | adv | — | instrumental of голова |
| голосом | /ˈɦɔlɔsɔm/ | with voice, in voice | adv | — | instrumental of голос |
| пошепки | /pɔˈʃɛpkɪ/ | in a whisper | adv | — | manner adverb |
| орудний | /oˈrudnɪj/ | Instrumental (case) | adj | m | this module |
| відмінок | /vidˈminɔk/ | grammatical case | noun | m | review |
| час | /t͡ʃɑs/ | time | noun | m | review from A1 |

---
