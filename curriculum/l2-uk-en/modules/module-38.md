---
module: 38
title: "Case Integration"
subtitle: "All 7 Cases Together"
level: A2
phase: A2.1
duration: 45
transliteration: partial
tags: [cases, integration, review, practice]
objectives:
  - Use all 7 cases in context
  - Build multi-case sentences
  - Identify cases in complex texts
  - Switch between cases fluently
grammar:
  - All case functions review
  - Multi-case sentence building
  - Case identification strategies
---

# Lesson Content

## warm-up

### The Complete System

Congratulations! You now know all 7 Ukrainian cases. Let's see them working together:

**One sentence, multiple cases:**
> Я дав книгу братові біля столу вчора.
> (I gave a book to my brother near the table yesterday.)

- **Я** - nominative (subject)
- **книгу** - accusative (direct object)
- **братові** - dative (to whom)
- **столу** - genitive (біля + gen)

Cases work together to create precise meaning. Today we practice using them all!

> 💡 **Did You Know?**
>
> Most Ukrainian sentences use 2-3 different cases. Native speakers don't think about cases consciously - they just "sound right." With practice, you'll develop this intuition too!

## presentation

### The 7 Cases: Complete Review

| Case | Question | Function | Example |
|------|----------|----------|---------|
| Nominative | Хто? Що? | Subject | **Брат** читає. |
| Genitive | Кого? Чого? | Possession, absence, quantities | Книга **брата**. |
| Dative | Кому? Чому? | To/for whom | Даю **братові**. |
| Accusative | Кого? Що? | Direct object | Бачу **брата**. |
| Instrumental | Ким? Чим? | With/by means | Йду з **братом**. |
| Locative | На/у кому/чому? | Location | Думаю про **брата**. |
| Vocative | - | Direct address | **Брате**! |

> ⚡ **Pro Tip: Case Recognition**
>
> To identify a case in a sentence:
> 1. Find the verb
> 2. Ask what question the noun answers
> 3. Check for prepositions (they determine case)
> 4. Look at the ending

### Multi-Case Sentences

**Simple (2 cases):**
- **Мама** (nom) готує **вечерю** (acc). - Mom is preparing dinner.
- **Я** (nom) йду до **магазину** (gen). - I'm going to the store.

**Medium (3 cases):**
- **Я** (nom) даю **подарунок** (acc) **сестрі** (dat). - I give a gift to sister.
- **Ми** (nom) живемо біля **річки** (gen) з **батьками** (instr). - We live near the river with parents.

**Complex (4+ cases):**
- **Я** (nom) написав **листа** (acc) **другові** (dat) про **подорож** (acc after про).
  - I wrote a letter to a friend about the trip.
- **Учитель** (nom) пояснює **учням** (dat) правила **граматики** (gen) **олівцем** (instr).
  - The teacher explains grammar rules to students with a pencil.

> 📜 **History Bite: Case-Rich Languages**
>
> Ukrainian's 7 cases come from Proto-Slavic, which had even more! Finnish has 15 cases, Hungarian has 18. Ukrainian strikes a balance - enough cases to be precise, not so many as to be overwhelming.

### Case Chains: Following the Logic

**Nominative starts the chain:**
Брат читає книгу сестрі.
- Хто читає? → Брат (nom)
- Що читає? → Книгу (acc)
- Кому читає? → Сестрі (dat)

**Genitive for possession/origin:**
Книга брата лежить на столі.
- Що лежить? → Книга (nom)
- Чия книга? → Брата (gen)
- Де лежить? → На столі (loc)

**Instrumental for means/company:**
Я їду з братом автобусом до Львова.
- Хто їде? → Я (nom)
- З ким? → З братом (instr)
- Чим? → Автобусом (instr)
- Куди? → До Львова (gen)

### Common Multi-Case Patterns

**Pattern 1: Subject + Action + Object + Recipient**
NOM + verb + ACC + DAT
- Батько дав гроші синові.
- Вчителька показала карту студентам.

**Pattern 2: Subject + Action + Object + Location**
NOM + verb + ACC + у/на + LOC
- Я поклав книгу на полицю.
- Вона поставила вазу у кімнаті.

**Pattern 3: Subject + Action + Companion + Means**
NOM + verb + з + INSTR + INSTR (means)
- Ми їдемо з друзями поїздом.
- Вона йде з сестрою пішки.

**Pattern 4: Subject + Action + Destination + Purpose**
NOM + verb + до + GEN + за + ACC
- Він пішов до магазину за хлібом.
- Я біжу до лікаря за допомогою.

> 🎭 **Culture Corner: Formal Requests**
>
> In formal Ukrainian, multi-case sentences show politeness:
> - "Прошу Вас передати цю інформацію директорові компанії."
> - (I ask you to pass this information to the company director.)
>
> The more precise, the more polite!

### Case Conflicts: Choosing Right

**Same verb, different cases by meaning:**

писати:
- Я пишу **листа** (acc). - I write a letter. (what)
- Я пишу **другові** (dat). - I write to a friend. (to whom)
- Я пишу **ручкою** (instr). - I write with a pen. (by what means)

дивитися:
- Я дивлюся **фільм** (acc). - I watch a film. (what)
- Я дивлюся **на тебе** (acc). - I look at you. (direction)
- Я дивлюся **з цікавістю** (instr). - I look with interest. (manner)

думати:
- Я думаю **про тебе** (acc). - I think about you.
- Я думаю **над проблемою** (instr). - I think over a problem.

> ⚡ **Pro Tip: When in Doubt**
>
> 1. Check the preposition - it usually determines case
> 2. Ask the case question (кого? кому? ким? etc.)
> 3. Think about the FUNCTION (object? recipient? means?)

### Reading Multi-Case Sentences

**Practice parsing:**

**Sentence 1:**
> Маленька дівчинка дала красивий малюнок своїй бабусі.

Parse:
- Хто? → маленька дівчинка (nom) - subject
- Що дала? → красивий малюнок (acc) - direct object
- Кому? → своїй бабусі (dat) - recipient

**Sentence 2:**
> Студенти з різних країн зустрілися біля університету перед лекцією.

Parse:
- Хто? → студенти (nom) - subject
- З яких країн? → з різних країн (gen) - origin
- Де? → біля університету (gen) - location
- Коли? → перед лекцією (instr) - time

**Sentence 3:**
> Мій друг, Іване, передай, будь ласка, цю книгу Марії від мене!

Parse:
- Іване (voc) - direct address
- Що передай? → цю книгу (acc) - object
- Кому? → Марії (dat) - recipient
- Від кого? → від мене (gen) - source

## practice

### Identify All Cases

Find and label each case in the sentences:

1. Мама готує смачний обід для сім'ї.
   - Мама (nom), обід (acc), сім'ї (gen)

2. Я розповів другові про свою подорож.
   - Я (nom), другові (dat), подорож (acc)

3. Студентка пише листа батькам ручкою.
   - Студентка (nom), листа (acc), батькам (dat), ручкою (instr)

4. Брате, дай мені книгу зі столу!
   - Брате (voc), мені (dat), книгу (acc), столу (gen)

### Complete Multi-Case Sentences

Fill in ALL blanks:

1. ___ (я) даю ___ (книга) ___ (сестра).
   > [!answer] Я даю книгу сестрі.

2. ___ (вчитель) пояснює ___ (правило) ___ (студенти).
   > [!answer] Вчитель пояснює правило студентам.

3. ___ (ми) йдемо з ___ (друзі) до ___ (парк).
   > [!answer] Ми йдемо з друзями до парку.

4. ___ (дідусь), розкажи ___ (я) про ___ (війна).
   > [!answer] Дідусю, розкажи мені про війну.

### Build Complex Sentences

Create sentences using all given elements:

1. [я / писати / лист / друг / ручка]
   > [!answer] Я пишу листа другові ручкою.

2. [батько / купити / подарунок / мама / магазин]
   > [!answer] Батько купив подарунок мамі в магазині.

3. [ми / зустрітися / колеги / кафе / після / робота]
   > [!answer] Ми зустрілися з колегами в кафе після роботи.

## production

### Real Situations

**Scenario 1: Telling a story**
- Вчора я пішов до парку з друзями.
- Ми сиділи біля озера і розмовляли про життя.
- Потім мій друг дав мені цікаву книгу.
- Тепер я читаю її з великим задоволенням.

**Scenario 2: Giving instructions**
- Візьми документи зі столу в кабінеті.
- Передай їх секретарці на другому поверсі.
- Скажи їй, що це від директора для бухгалтерії.
- Після цього повернися до мене.

**Scenario 3: Describing a day**
- Зранку я їду автобусом на роботу.
- Протягом дня я працюю з клієнтами.
- Після обіду я пишу листи партнерам.
- Увечері зустрічаюся з друзями біля кінотеатру.

---

---

---

---

# Activities

## fill-in: All 7 Cases Practice

> Choose the correct case form to complete each sentence. Identify which case is needed.

1. Я бачу ___ (моя сестра) в кафе кожного ранку.
   > [!answer] мою сестру
   > [!options] моя сестра | моєї сестри | моїй сестрі | мою сестру

2. Книга належить ___ (мій друг) з дитинства.
   > [!answer] моєму другові
   > [!options] мій друг | мого друга | моєму другові | моїм другом

3. Ми живемо біля ___ (центральний парк) у Києві.
   > [!answer] центрального парку
   > [!options] центральний парк | центрального парку | центральному парку | центральним парком

4. Вона працює ___ (вчитель) у новій школі.
   > [!answer] вчителем
   > [!options] вчитель | вчителя | вчителю | вчителем

5. Книги лежать на ___ (великий стіл) у вітальні.
   > [!answer] великому столі
   > [!options] великий стіл | великого стола | великому столі | великим столом

6. Дай, будь ласка, цю каву ___ (мій колега)!
   > [!answer] моєму колезі
   > [!options] мій колега | мого колегу | моєму колезі | моїм колегою

7. Ти знаєш ___ (ця жінка) з нашого офісу?
   > [!answer] цю жінку
   > [!options] ця жінка | цієї жінки | цій жінці | цю жінку

8. Я їду автобусом до ___ (робота) кожного дня.
   > [!answer] роботи
   > [!options] робота | роботи | роботі | роботою

9. ___ (Оксана), допоможи мені, будь ласка!
   > [!answer] Оксано
   > [!options] Оксана | Оксани | Оксані | Оксано

10. Він гуляє з ___ (свій собака) у парку.
    > [!answer] своїм собакою
    > [!options] свій собака | свого собаку | своєму собаці | своїм собакою

## unjumble: Build the Sentence

> Arrange the words to form multi-case sentences.

1. сестру / Я / бачу / мою / в кафе / кожного ранку
   > [!answer] Я бачу мою сестру в кафе кожного ранку.
   > (I see my sister in the cafe every morning. - Accusative)

2. другові / Книга / належить / моєму / з дитинства
   > [!answer] Книга належить моєму другові з дитинства.
   > (The book belongs to my friend since childhood. - Dative)

3. парку / Ми / живемо / біля / центрального / у Києві
   > [!answer] Ми живемо біля центрального парку у Києві.
   > (We live near the central park in Kyiv. - Genitive)

4. вчителем / Вона / працює / у новій школі
   > [!answer] Вона працює вчителем у новій школі.
   > (She works as a teacher at a new school. - Instrumental)

5. столі / Книги / лежать / на / великому / у вітальні
   > [!answer] Книги лежать на великому столі у вітальні.
   > (Books lie on a big table in the living room. - Locative)

6. колезі / Дай / цю каву / моєму / будь ласка
   > [!answer] Дай, будь ласка, цю каву моєму колезі!
   > (Please give this coffee to my colleague! - Dative)

7. жінку / Ти / знаєш / цю / з нашого офісу
   > [!answer] Ти знаєш цю жінку з нашого офісу?
   > (Do you know this woman from our office? - Accusative)

8. роботи / Я / їду / до / автобусом / кожного дня
   > [!answer] Я їду автобусом до роботи кожного дня.
   > (I go by bus to work every day. - Genitive + Instrumental)

9. Оксано / допоможи / мені / будь ласка
   > [!answer] Оксано, допоможи мені, будь ласка!
   > (Oksana, help me please! - Vocative + Dative)

10. собакою / Він / гуляє / з / своїм / у парку
    > [!answer] Він гуляє з своїм собакою у парку.
    > (He walks with his dog in the park. - Instrumental + Locative)

## match-up: Match the Pairs

> Match Ukrainian words to their English meanings.

| Left | Right |
|------|-------|
| кабінет | office, study |
| ваза | vase |
| малюнок | drawing, picture |
| протягом | during, throughout |
| бухгалтерія | accounting dept |
| партнер | partner |
| задоволення | pleasure, satisfaction |
| іноді | sometimes |
| правило | rule |
| принести | to bring |
| полиця | shelf |
| ви | you (formal/plural) |

## quiz: Meaning Check

> Choose the correct answer.

1. What does "секретарка" mean?
   - [ ] straight
   - [x] secretary (f)
   - [ ] eyes
   - [ ] Accusative: бачити, читати, любити
   > "секретарка" means "secretary (f)"

2. What does "задоволення" mean?
   - [ ] to speak
   - [ ] lecture
   - [x] pleasure, satisfaction
   - [ ] to become
   > "задоволення" means "pleasure, satisfaction"

3. What does "поверх" mean?
   - [ ] to sell
   - [ ] Kyiv
   - [x] floor (of building)
   - [ ] experience
   > "поверх" means "floor (of building)"

4. What does "малюнок" mean?
   - [ ] calf
   - [x] drawing, picture
   - [ ] to be considered
   - [ ] to be lucky
   > "малюнок" means "drawing, picture"

5. What does "бухгалтерія" mean?
   - [ ] to understand
   - [ ] in the middle
   - [ ] price
   - [x] accounting dept
   > "бухгалтерія" means "accounting dept"

6. What does "полиця" mean?
   - [x] shelf
   - [ ] to inform
   - [ ] thin
   - [ ] pork fat
   > "полиця" means "shelf"

7. What does "партнер" mean?
   - [ ] that
   - [ ] cucumber
   - [ ] there
   - [x] partner
   > "партнер" means "partner"

8. What does "лекція" mean?
   - [ ] small
   - [ ] lemon
   - [ ] English (adj)
   - [x] lecture
   > "лекція" means "lecture"

9. What does "протягом" mean?
   - [ ] to have
   - [ ] neighbor (f)
   - [x] during, throughout
   - [ ] salt
   > "протягом" means "during, throughout"

10. What does "правило" mean?
   - [ ] gift
   - [ ] we
   - [x] rule
   - [ ] to be going to
   > "правило" means "rule"

11. What does "ваза" mean?
   - [x] vase
   - [ ] potato
   - [ ] to remain
   - [ ] entrepreneur
   > "ваза" means "vase"

12. What does "кабінет" mean?
   - [ ] to try on
   - [x] office, study
   - [ ] discount
   - [ ] porridge
   > "кабінет" means "office, study"

## true-false: True or False?

> Determine if each statement is true or false.

- [x] "кабінет" means "office, study"
   > Correct!

- [x] "лекція" means "lecture"
   > Correct!

- [ ] "ваза" means "can, to be able"
   > Incorrect. "ваза" means "vase"

- [x] "малюнок" means "drawing, picture"
   > Correct!

- [x] "протягом" means "during, throughout"
   > Correct!

- [x] "партнер" means "partner"
   > Correct!

- [x] "секретарка" means "secretary (f)"
   > Correct!

- [x] "правило" means "rule"
   > Correct!

- [ ] "полиця" means "yesterday"
   > Incorrect. "полиця" means "shelf"

- [x] "бухгалтерія" means "accounting dept"
   > Correct!

## group-sort: Sort by Gender

> Sort the nouns into gender categories.

### Masculine
- кабінет
- малюнок
- партнер
- поверх

### Feminine
- полиця
- ваза
- бухгалтерія
- секретарка
- лекція

### Neuter
- задоволення
- правило

## select: Choose Cases and Functions

> Select all words that belong to the category.

1. Which are nominative case functions?
   - [x] Subject of sentence
   - [ ] Direct object
   - [ ] Possession
   - [x] Predicate noun
   - [ ] To/for whom
   - [ ] Location
   - [ ] With/by means
   - [ ] Direct address
   - [ ] After preposition без
   - [ ] After preposition з

2. Which are accusative case functions?
   - [ ] Subject
   - [x] Direct object
   - [ ] Possession
   - [x] Motion destination (у, на + acc)
   - [ ] To/for whom
   - [ ] Location
   - [ ] With/by means
   - [ ] Direct address
   - [ ] After до
   - [ ] After від

3. Which are genitive case functions?
   - [ ] Subject
   - [ ] Direct object
   - [x] Possession
   - [ ] Motion destination
   - [x] After без
   - [x] After до
   - [x] After від
   - [x] Quantities
   - [x] Absence/negation
   - [ ] With/by means

4. Which are dative case functions?
   - [ ] Subject
   - [ ] Direct object
   - [ ] Possession
   - [x] To/for whom (indirect object)
   - [ ] Location
   - [ ] With/by means
   - [x] Giving/telling someone
   - [ ] Direct address
   - [ ] After без
   - [ ] After з

5. Which are instrumental case functions?
   - [ ] Subject
   - [ ] Direct object
   - [ ] Possession
   - [ ] Motion destination
   - [x] With someone (companionship)
   - [x] By means of (tool/transport)
   - [x] Manner of doing
   - [x] After preposition з (with)
   - [x] Profession (with працювати, стати)
   - [ ] After до

6. Which are locative case functions?
   - [ ] Subject
   - [ ] Direct object
   - [ ] Possession
   - [x] Location (де?)
   - [x] After у/в (location)
   - [x] After на (location)
   - [x] After про (about)
   - [ ] With/by means
   - [ ] Direct address
   - [ ] After від

7. Which questions identify accusative case?
   - [ ] Хто?
   - [x] Кого? (animate)
   - [x] Що? (inanimate)
   - [ ] Кому?
   - [ ] Чому?
   - [x] Куди? (where to)
   - [ ] Де?
   - [ ] Чим?
   - [ ] Ким?
   - [ ] Кого? (possession)

8. Which questions identify genitive case?
   - [ ] Хто?
   - [x] Кого? (possession)
   - [x] Чого? (possession)
   - [ ] Кому?
   - [ ] Що? (direct object)
   - [ ] Куди?
   - [ ] Де?
   - [x] Звідки? (from where)
   - [ ] Ким?
   - [ ] Чим?

9. Which require genitive case?
   - [x] біля парку
   - [ ] у парку
   - [x] до школи
   - [x] від вокзалу
   - [x] для мами
   - [x] без води
   - [ ] з другом
   - [x] після уроку
   - [ ] над містом
   - [ ] перед будинком

10. Which require instrumental case?
    - [ ] біля парку
    - [ ] до школи
    - [x] з другом
    - [ ] від вокзалу
    - [ ] для мами
    - [ ] без води
    - [x] над містом
    - [x] перед будинком
    - [x] між деревами
    - [x] автобусом (transport)

## unjumble: Arrange Words into Sentences

> Put the words in the correct order to form grammatically correct sentences.

1. книгу / братові / Я / дав / столу / біля
   > [!answer] Я дав книгу братові біля столу.
   > (I gave a book to brother near the table.)

2. готує / Мама / вечерю / кухні / на
   > [!answer] Мама готує вечерю на кухні.
   > (Mom prepares dinner in the kitchen.)

3. подарунок / сестрі / даю / Я / день / народження / на
   > [!answer] Я даю подарунок сестрі на день народження.
   > (I give a gift to sister for birthday.)

4. живемо / Ми / річки / біля / батьками / з
   > [!answer] Ми живемо біля річки з батьками.
   > (We live near the river with parents.)

5. листа / Я / другові / написав / подорож / про
   > [!answer] Я написав листа другові про подорож.
   > (I wrote a letter to friend about the trip.)

6. Учитель / учням / правила / пояснює / граматики
   > [!answer] Учитель пояснює учням правила граматики.
   > (The teacher explains grammar rules to students.)

7. книга / брата / на / лежить / столі
   > [!answer] Книга брата лежить на столі.
   > (Brother's book lies on the table.)

8. школу / Вона / до / автобусом / їде
   > [!answer] Вона їде до школи автобусом.
   > (She goes to school by bus.)

9. думає / про / екзамен / студент / кімнаті / у
   > [!answer] Студент думає про екзамен у кімнаті.
   > (The student thinks about the exam in the room.)

10. працює / Брат / лікарем / лікарні / у / Києві / у
    > [!answer] Брат працює лікарем у лікарні у Києві.
    > (Brother works as a doctor at a hospital in Kyiv.)

---

# Vocabulary

| Word | IPA | English | POS | Gender | Note |
|------|-----|---------|-----|--------|------|
| правило | /prɑˈvɪlo/ | rule | noun | n |
| полиця | /poˈlɪtsʲɑ/ | shelf | noun | f |
| ваза | /ˈvɑzɑ/ | vase | noun | f |
| поверх | /ˈpovɛrx/ | floor (of building) | noun | m |
| кабінет | /kɑbʲiˈnɛt/ | office, study | noun | m |
| секретарка | /sɛkrɛˈtɑrkɑ/ | secretary (f) | noun | f |
| бухгалтерія | /buxɦɑlˈtɛrʲijɑ/ | accounting dept | noun | f |
| партнер | /pɑrtˈnɛr/ | partner | noun | m |
| малюнок | /mɑˈlʲunok/ | drawing, picture | noun | m |
| лекція | /ˈlɛktsʲijɑ/ | lecture | noun | f |
| задоволення | /zɑdoˈvolɛnʲːɑ/ | pleasure, satisfaction | noun | n |
| протягом | /proˈtʲɑɦom/ | during, throughout | prep | - | + gen |

---

# Summary

## The 7 Cases at a Glance

| Case | Question | Typical Use |
|------|----------|-------------|
| Nominative | Хто? Що? | Subject |
| Genitive | Кого? Чого? | Possession, absence, "of" |
| Dative | Кому? | To/for whom |
| Accusative | Кого? Що? | Direct object |
| Instrumental | Ким? Чим? | With, by means of |
| Locative | Де? У чому? | Location (with preposition) |
| Vocative | - | Calling someone |

## Multi-Case Sentence Patterns

| Pattern | Example |
|---------|---------|
| NOM + ACC | Я читаю книгу. |
| NOM + ACC + DAT | Я даю книгу братові. |
| NOM + з + INSTR | Я йду з другом. |
| NOM + до + GEN | Я йду до магазину. |
| NOM + у/на + LOC | Я живу в Києві. |

## Case Identification Strategy

1. Find the verb (or preposition)
2. Ask: Хто? Що? Кого? Кому? Ким? Де?
3. Check the ending
4. Consider the function

> 💡 **Key Takeaways**
>
> - Most sentences use 2-4 different cases
> - Prepositions determine case for nouns that follow
> - Practice identifying cases in real sentences
> - The question words (хто, кого, кому, ким) match case questions
> - Context helps when endings are similar
