---
module: a2-12
title: "The Completed Past"
subtitle: "Perfective Verbs in Past Tense"
version: "1.0"
phase: "A2.2"
pedagogy: "PPP"
duration: "60 min"
transliteration: none
tags:
  - grammar
  - aspect
  - past-tense
  - perfective
grammar:
  - perfective past tense
  - completion vs process
  - aspect choice in past
objectives:
  - "Learner can conjugate perfective verbs in past tense"
  - "Learner understands when to use perfective past"
  - "Learner can express completed actions"
  - "Learner can distinguish process from result in past"
vocabulary_count: 20
---
# Warm-up

Now that you understand the concept of aspect, let's focus on using perfective verbs in the past tense. This is where aspect becomes most important — choosing perfective or imperfective past can completely change your meaning!

**What you'll learn:**
- How to form perfective past tense
- When to choose perfective over imperfective
- Key markers that signal completed actions

**Remember:** Perfective past = completed action with a clear result

---

# Presentation

## Forming the Perfective Past

The good news: perfective verbs conjugate in past tense exactly like imperfective verbs! The past tense endings are the same for both aspects.

### Past Tense Endings (Same for Both Aspects)

| Gender/Number | Ending | Example (прочитати) |
|---------------|--------|---------------------|
| Masculine | -в / -∅ | прочитав |
| Feminine | -ла | прочитала |
| Neuter | -ло | прочитало |
| Plural | -ли | прочитали |

**Examples:**
- Він **прочитав** книгу. (He read/finished the book.)
- Вона **прочитала** статтю. (She read/finished the article.)
- Вони **прочитали** все. (They read/finished everything.)

> 💡 **Key Point**
>
> The difference isn't in the endings — it's in the verb itself. Прочитав (perfective) vs. читав (imperfective) have the same endings, but different stems.

## Imperfective vs. Perfective Past: Side by Side

Let's compare the same verb pair in past tense:

| Aspect | Masculine | Feminine | Plural | Meaning |
|--------|-----------|----------|--------|---------|
| Impf. | писав | писала | писали | was writing |
| Perf. | написав | написала | написали | wrote/finished |
| Impf. | читав | читала | читали | was reading |
| Perf. | прочитав | прочитала | прочитали | read/finished |
| Impf. | робив | робила | робили | was doing |
| Perf. | зробив | зробила | зробили | did/finished |

## When to Use Perfective Past

### 1. Single Completed Action

When something happened once and is finished:
- Я **прочитав** книгу вчора. (I finished the book yesterday.)
- Вона **написала** листа. (She wrote/finished the letter.)
- Він **з'їв** сніданок. (He ate breakfast.)

### 2. Result Matters More Than Process

Focus on what got accomplished:
- Хто **написав** цю книгу? (Who wrote this book?)
- Вони **побудували** новий міст. (They built a new bridge.)

### 3. With "Already" (вже) and "Finally" (нарешті)

These words almost always signal perfective:
- Я **вже прочитав** цю книгу. (I've already read this book.)
- Вона **нарешті зробила** домашнє завдання. (She finally did her homework.)

> 📜 **Pattern Recognition**
>
> Words like вже, нарешті, раптом, несподівано often appear with perfective verbs because they emphasize completed events or results.

## When to Use Imperfective Past

### 1. Duration or Process

When the action lasted for some time:
- Я **читав** книгу три години. (I was reading for 3 hours.)
- Вона **писала** весь день. (She was writing all day.)

### 2. Repeated Actions

When something happened multiple times:
- Він **читав** цю книгу багато разів. (He read this book many times.)
- Ми **бачили** його часто. (We saw him often.)

### 3. Asking About Activities

When asking "what were you doing?":
- Що ти **робив** учора? (What were you doing yesterday?)
- Що вона **читала**? (What was she reading?)

> [!tip] Decision Guide
> Ask yourself: "Am I focusing on what got done (perfective) or what someone was doing (imperfective)?"

---

# Cultural Insight

## Common Perfective Past Forms

Here are the most common perfective verbs in past tense:

| Perfective | Past (masc.) | English |
|------------|--------------|---------|
| зробити | зробив | did/made |
| написати | написав | wrote |
| прочитати | прочитав | read/finished |
| сказати | сказав | said |
| побачити | побачив | saw |
| почути | почув | heard |
| взяти | взяв | took |
| дати | дав | gave |
| піти | пішов | went/left |
| прийти | прийшов | came |
| приїхати | приїхав | arrived |
| вийти | вийшов | went out |
| увійти | увійшов | entered |
| почати | почав | began |
| закінчити | закінчив | finished |

> 🔍 **Irregular Forms**
>
> Some perfective verbs have irregular past forms: піти → пішов (not "пів"), прийти → прийшов.

---

# Practice

## Practice Examples

Choose perfective or imperfective for these contexts:

1. **I finished writing the letter.** → Я **написав** листа. (perf.)
2. **I was writing for 2 hours.** → Я **писав** дві години. (impf.)
3. **She said "yes."** → Вона **сказала** "так." (perf.)
4. **He was speaking loudly.** → Він голосно **говорив**. (impf.)
5. **We already read this.** → Ми вже це **прочитали**. (perf.)
6. **They were reading all evening.** → Вони **читали** весь вечір. (impf.)

---

# Summary

## What You Learned

This module focused on using perfective verbs in past tense to express completed actions.

**Key Points:**
- Perfective and imperfective use the SAME past tense endings (-в, -ла, -ло, -ли)
- The difference is in the verb stem, not the ending
- Use **perfective past** for completed actions with results
- Use **imperfective past** for processes, duration, or repeated actions
- Signal words: вже, нарешті = usually perfective; довго, часто = usually imperfective

Тепер ви можете говорити про завершені дії! ✅

# Activities

## match-up: Past Tense Forms

- зробив :: did/finished
- написав :: wrote/finished
- прочитав :: read/finished
- сказав :: said
- побачив :: saw
- почув :: heard
- взяв :: took
- дав :: gave
- пішов :: went/left
- прийшов :: came/arrived
- закінчив :: finished
- почав :: began

## fill-in: Perfective or Imperfective Past

> Choose the correct verb form for each context.

1. Він ___ книгу вчора. (finished reading)
   > [!answer] прочитав
   > [!options] читав | прочитав

2. Вона ___ три години. (was writing)
   > [!answer] писала
   > [!options] писала | написала

3. Я вже ___ це! (already did)
   > [!answer] зробив
   > [!options] робив | зробив

4. Ми ___ щодня. (read every day)
   > [!answer] читали
   > [!options] читали | прочитали

5. Він ___ "привіт." (said)
   > [!answer] сказав
   > [!options] говорив | сказав

6. Вона довго ___. (was speaking)
   > [!answer] говорила
   > [!options] говорила | сказала

7. Ми нарешті ___! (finally arrived)
   > [!answer] приїхали
   > [!options] їхали | приїхали

8. Він ___ весь день. (was working)
   > [!answer] працював
   > [!options] працював | попрацював

9. Вона ___ роботу. (finished)
   > [!answer] закінчила
   > [!options] закінчувала | закінчила

10. Я ___ його часто. (saw often)
    > [!answer] бачив
    > [!options] бачив | побачив

11. Він раптом ___ шум. (suddenly heard)
    > [!answer] почув
    > [!options] чув | почув

12. Ми ___ листа. (wrote/finished)
    > [!answer] написали
    > [!options] писали | написали

## quiz: Aspect in Past Tense

1. "Я прочитав книгу" focuses on:

- [x] Completed result
- [ ] Ongoing process

---

2. "Я читав книгу" focuses on:

- [x] Process
- [ ] Completed result

---

3. Which word signals perfective past?

- [x] вже (already)
- [ ] довго (for a long time)

---

4. "Вона писала три години" — aspect?

- [x] Imperfective
- [ ] Perfective

---

5. "Він написав листа вчора" — aspect?

- [x] Perfective
- [ ] Imperfective

---

6. Past tense endings are:

- [x] The same for both aspects
- [ ] Different for each aspect

---

7. Masculine past ending:

- [x] -в / -∅
- [ ] -ла
- [ ] -ли

---

8. Feminine past ending:

- [x] -ла
- [ ] -в
- [ ] -ли

---

9. "Що ти робив?" uses:

- [x] Imperfective (asking about activities)
- [ ] Perfective

---

10. "Ти зробив це?" uses:

- [x] Perfective (asking about completion)
- [ ] Imperfective

---

11. "Піти" past masculine:

- [x] пішов
- [ ] пів
- [ ] пишов

---

12. "Прийти" past masculine:

- [x] прийшов
- [ ] прийв
- [ ] прив

## true-false: Past Tense Rules

> Decide if each statement is true or false.

- [x] Perfective past shows completed actions.
  > Correct!

- [x] Imperfective past can show duration.
  > Yes! "Він читав три години."

- [ ] Past endings are different for each aspect.
  > No, the endings are the same — the stems are different.

- [x] "Вже" usually appears with perfective.
  > Correct! "Я вже зробив."

- [x] "Довго" usually appears with imperfective.
  > Yes! "Він довго працював."

- [ ] "Писав" is perfective past.
  > "Писав" is imperfective. "Написав" is perfective.

- [x] "Прочитав" means "finished reading."
  > Correct!

- [x] "Пішов" is the past of "піти."
  > Yes!

- [ ] Repeated actions use perfective.
  > Repeated actions use imperfective.

- [x] "Що ти робив?" asks about activities.
  > Yes, using imperfective to ask about process.

- [x] "Нарешті" signals perfective.
  > Correct! "Нарешті зробив!"

- [ ] "Сказав" is imperfective.
  > "Сказав" is perfective. "Говорив" is imperfective.

## group-sort: Verb Aspect

> Sort these past tense forms by aspect.

### Imperfective
- читав
- писав
- робив
- говорив
- бачив
- їхав

### Perfective
- прочитав
- написав
- зробив
- сказав
- побачив
- приїхав

## unjumble: Past Tense Sentences

> Put the words in the correct order.

1. він / прочитав / книгу / вчора
   > [!answer] Він прочитав книгу вчора.

2. вона / писала / три / години
   > [!answer] Вона писала три години.

3. я / вже / зробив / це
   > [!answer] Я вже зробив це.

4. ми / нарешті / приїхали
   > [!answer] Ми нарешті приїхали.

5. він / сказав / правду
   > [!answer] Він сказав правду.

6. вона / довго / говорила
   > [!answer] Вона довго говорила.

7. ти / прочитав / статтю
   > [!answer] Ти прочитав статтю?

8. він / пішов / додому
   > [!answer] Він пішов додому.

9. вона / побачила / море
   > [!answer] Вона побачила море.

10. ми / закінчили / роботу
    > [!answer] Ми закінчили роботу.

11. він / раптом / почув / шум
    > [!answer] Він раптом почув шум.

12. я / взяв / книгу
    > [!answer] Я взяв книгу.

## error-correction: Choose Correct Aspect

> Select the correct aspect for each context.

1. Він ___ книгу весь вечір. (duration)
   > [!error] duration
   > [!answer] читав
   > [!explanation] Duration (весь вечір) requires imperfective.
   > [!options] читав | прочитав

2. Вона ___ листа і пішла. (completed then left)
   > [!error] completed then left
   > [!answer] написала
   > [!explanation] Completed action before next action = perfective.
   > [!options] писала | написала

3. Я вже ___ це! (already)
   > [!error] already
   > [!answer] зробив
   > [!explanation] "Вже" signals completion = perfective.
   > [!options] робив | зробив

4. Він ___ часто. (often)
   > [!error] often
   > [!answer] приходив
   > [!explanation] Repeated action (часто) = imperfective.
   > [!options] приходив | прийшов

5. Вона нарешті ___! (finally)
   > [!error] finally
   > [!answer] зробила
   > [!explanation] "Нарешті" signals completion = perfective.
   > [!options] робила | зробила

6. Що ти ___ учора? (activities)
   > [!error] activities
   > [!answer] робив
   > [!explanation] Asking about activities = imperfective.
   > [!options] робив | зробив

## cloze: Aspect in Context

> Fill in the correct past tense form.

Учора я [___:1] рано. Я [___:2] каву і [___:3] сніданок. Потім я [___:4] на роботу. На роботі я [___:5] цілий день. Увечері я [___:6] додому і [___:7] телевізор.

1. встати: встав / вставав
> [!answer] встав

2. випити: пив / випив
> [!answer] випив

3. з'їсти: їв / з'їв
> [!answer] з'їв

4. піти: ходив / пішов
> [!answer] пішов

5. працювати: працював / попрацював
> [!answer] працював

6. прийти: приходив / прийшов
> [!answer] прийшов

7. дивитися: дивився / подивився
> [!answer] дивився

---

Минулого тижня я [___:1] нову книгу. Я [___:2] її три дні. Вчора я нарешті [___:3] читати. Книга була дуже цікава! Після того як я [___:4] книгу, я [___:5] рецензію.

1. купити: купував / купив
> [!answer] купив

2. читати: читав / прочитав
> [!answer] читав

3. закінчити: закінчував / закінчив
> [!answer] закінчив

4. читати: читав / прочитав
> [!answer] прочитав

5. писати: писав / написав
> [!answer] написав

## dialogue-reorder: Completed vs Process

- Що ти робив учора?
- Я працював цілий день.
- Ти закінчив проект?
- Так, нарешті закінчив!
- А що робила Оля?
- Вона теж працювала, але не закінчила.

> [!answer]
> 1. Що ти робив учора?
> 2. Я працював цілий день.
> 3. Ти закінчив проект?
> 4. Так, нарешті закінчив!
> 5. А що робила Оля?
> 6. Вона теж працювала, але не закінчила.

---

- Ти прочитав книгу?
- Ні, я читав, але не закінчив.
- Скільки часу ти читав?
- Дві години.
- А коли закінчиш?
- Думаю, сьогодні прочитаю.

> [!answer]
> 1. Ти прочитав книгу?
> 2. Ні, я читав, але не закінчив.
> 3. Скільки часу ти читав?
> 4. Дві години.
> 5. А коли закінчиш?
> 6. Думаю, сьогодні прочитаю.

## mark-the-words: Find Perfective Past

Click on PERFECTIVE PAST forms only.

Учора я [прочитав](correct) книгу. Потім я [написав](correct) листа. Я довго писав, але нарешті [закінчив](correct).

---

Вона [сказала](correct) правду. Ми [почули](correct) шум. Він [взяв](correct) книгу і [пішов](correct).

---

Він працював цілий день. Потім він [закінчив](correct) і [пішов](correct) додому. Вдома він [відпочив](correct).

## translate: English to Ukrainian

He finished reading the book.

- [x] Він прочитав книгу.
- [ ] Він читав книгу.

---

She was writing for 3 hours.

- [x] Вона писала три години.
- [ ] Вона написала три години.

---

I already did this!

- [x] Я вже зробив це!
- [ ] Я вже робив це!

---

We finally arrived.

- [x] Ми нарешті приїхали.
- [ ] Ми нарешті їхали.

---

He said "yes."

- [x] Він сказав "так."
- [ ] Він говорив "так."

---

She was speaking loudly.

- [x] Вона голосно говорила.
- [ ] Вона голосно сказала.

---

They saw the movie.

- [x] Вони побачили фільм.
- [ ] Вони бачили фільм.

---

I was working all day.

- [x] Я працював цілий день.
- [ ] Я попрацював цілий день.

---

He took the book and left.

- [x] Він взяв книгу і пішов.
- [ ] Він брав книгу і йшов.

---

She heard the news.

- [x] Вона почула новини.
- [ ] Вона чула новини.

---

What were you doing yesterday?

- [x] Що ти робив учора?
- [ ] Що ти зробив учора?

---

Did you finish? (asking about completion)

- [x] Ти закінчив?
- [ ] Ти закінчував?

---

# Vocabulary

| Word | IPA | English | POS | Gender | Note |
|------|-----|---------|-----|--------|------|
| зробив | /zrɔˈbɪʋ/ | did, made | verb | m | perf. past masc. |
| написав | /nɑpɪˈsɑʋ/ | wrote | verb | m | perf. past masc. |
| прочитав | /prɔt͡ʃɪˈtɑʋ/ | read, finished | verb | m | perf. past masc. |
| сказав | /skɑˈzɑʋ/ | said | verb | m | perf. past masc. |
| побачив | /pɔˈbɑt͡ʃɪʋ/ | saw | verb | m | perf. past masc. |
| почув | /pɔˈt͡ʃuʋ/ | heard | verb | m | perf. past masc. |
| взяв | /ˈʋzʲɑʋ/ | took | verb | m | perf. past masc. |
| дав | /dɑʋ/ | gave | verb | m | perf. past masc. |
| пішов | /piˈʃɔʋ/ | went, left | verb | m | perf. past masc. |
| прийшов | /prɪjˈʃɔʋ/ | came | verb | m | perf. past masc. |
| приїхав | /prɪˈjixɑʋ/ | arrived | verb | m | perf. past masc. |
| вийшов | /ˈʋɪjʃɔʋ/ | went out | verb | m | perf. past masc. |
| увійшов | /uˈʋijʃɔʋ/ | entered | verb | m | perf. past masc. |
| почав | /pɔˈt͡ʃɑʋ/ | began | verb | m | perf. past masc. |
| закінчив | /zɑˈkint͡ʃɪʋ/ | finished | verb | m | perf. past masc. |
| зрозумів | /zrɔzuˈmiʋ/ | understood | verb | m | perf. past masc. |
| забув | /zɑˈbuʋ/ | forgot | verb | m | perf. past masc. |
| знайшов | /znɑjˈʃɔʋ/ | found | verb | m | perf. past masc. |
| загубив | /zɑɦuˈbɪʋ/ | lost | verb | m | perf. past masc. |
| вже | /ʋʒɛ/ | already | adv | — | signals completion |

---
